...
forward_kinematics = @(theta1, theta2, L3) [L1 * cos(theta1) + L2 * cos(theta1 + theta2) + L3 * cos(theta1 + theta2 + theta3); 
...
while(abs(error_x) > tolerance && abs(error_y) > tolerance)
    ...
    current_pos = forward_kinematics(theta1, theta2, L3);
